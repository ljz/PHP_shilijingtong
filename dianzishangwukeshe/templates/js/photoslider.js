...
FOTO.Slider = {
	...
	getUrlFromId: function(key,id,isThumb){
		...
		if(isThumb){
			//修改为完整的缩略图文件路径
			return this.baseURL+"/"+this.thumbFileURL+"/"+this.thumbURL.replace('{ID}',id+".jpg");
		}else{
			//修改为完整的文件路径
			return this.baseURL+"/"+this.fileURL+"/"+this.mainURL.replace('{ID}',id);
		}
	},
